...
public class MixinHiresModelManager {
    ...
    void renderModel(HiresModelRenderer instance, World world, Vector3i min, Vector3i max, TileModel model,
        TileMetaConsumer tmc
    ) {
    ...
    private void valkyrienskies$transformModel(final int start, final int end, final TileModel model, final Vector3dc preTranslation, final Vector3dc postTranslation, final Matrix4dc transform) {
        final var positions = ((TileModelAccessor) model).getPositions();
        ...
        for(int face = start; face < end; ++face) {
            for(int i = 0; i < 3; ++i) {
                ...
                positions[index]     = (float) ((x * transform.m00()) + (y * transform.m10()) + (z * transform.m20()) + transform.m30() + postTranslation.x());
                positions[index + 1] = (float) ((x * transform.m01()) + (y * transform.m11()) + (z * transform.m21()) + transform.m31() + postTranslation.y());
                positions[index + 2] = (float) ((x * transform.m02()) + (y * transform.m12()) + (z * transform.m22()) + transform.m32() + postTranslation.z());
            }
        }
    }
}
